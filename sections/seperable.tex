...
\label{fig:notched}
\end{figure}
The box-plot was then expanded out from descriptive statistics to facilitating
...
which is a technique that only works well for relatively small sample sizes ($1\<1000$), at larger scales the widths are about equal. Instead, notches, as seen
...
via moving windowed average) proposed by Chambers et. al \cite{chambers1983}, as seen in Figure~\ref{fig:violinplot}%%is density
...
close these functions are to each other-functional depth\cite{fabrero2007}, bivariate score depth\cite{hyndman2009}, and
...
\begin{figure}
\includegraphics{contour_weather}
\label{fig:countour}
\end{figure}
